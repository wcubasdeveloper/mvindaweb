...
<!-- <link href="<?=base_url()?>assets/csoftcontent/css/components.min.css" rel="stylesheet" /> -->
...
<!-- <script src="<?=base_url()?>assets/csoftcontent/js/sweet_alert.min.js"></script> -->
   ...
   <div class="row wrapper border-bottom white-bg page-heading">
      <div class="col-lg-12">
        <h2>Asistencias&nbsp;&nbsp;&nbsp;<strong id="StnCantidadRegistros">-</strong>
          <input id="TxtFechaProgramacion" readonly class="FechaUI" style="width:130px;"/>
          ...
          <form class="form-vertical">
            ...
            <div class="row clearfix">
              ...
              <div class="col-md-9">
                <div class="form-group">
                  <div class="form-line">
                    <select class="form-control m-b" id="selectCodAsistenciaTipo" disabled>
                    </select>
                  </div>
                </div>
              </div>
...
<script>
  var TxtFechaProgramacion = $('#TxtFechaProgramacion');
  var DivPersonaProgramacion = $('#DivPersonaProgramacion');
  var selectCodPersona = $('#selectCodPersona');
  ...
  $(document).ready(function(){
    TxtFechaProgramacion.val(FechaHoraHoy(1)).change(function(){
  ...
  function ListarPersonaProgramacion() {
    ...
    var FechaProgramacion = FechaMySQL(TxtFechaProgramacion.val());
    var Parametros = FechaProgramacion;
    var Indice = 11;
    ...
    $.post(URL, Data, function (Data) {
      //
      DivPersonaProgramacion.empty();
      var FilasHTML = '';
      var CantidadFilas = Data.length;
      var BotonRegistrarAsistencia;
      //
      Html = '<div class="row">';
      $.each(Data, function(i) {
        switch (this.EstadoAsistencia) {
          case '1':
            BotonRegistrarAsistencia = 'Marcar entrada';
            break;
          case '2':
            BotonRegistrarAsistencia = 'Marcar salida';
            break;
          case '3':
            BotonRegistrarAsistencia = 'Finalizado';
            break;
        }
        //
        Html += 
        ...
        // '     </div>' +
        '       <p class="font-bold">Prog: ' + this.HoraInicioProgra + ' - ' + this.HoraFinProgra + '</p>' +
        '       <p class="font-bold">Reg: ' + this.HoraInicioReal + ' - ' + this.HoraFinReal + '</p>' +
        '       <p class="font-bold"><span class="label label-success">Básico: ' + this.ImporteBasico + '</span> <span class="label label-success">Comisión: ' + this.ImporteComision + '</span></p>' +
        '       <p class="font-bold"><span class="label label-danger">Dscto: ' + this.ImporteDescuento + '</span></p>' +
        '     <div class="text-center">' +
        //'       <a class="btn btn-xs btn-white"><i class="fa fa-thumbs-up"></i> Like </a>' +
        '       <a class="btn btn-xs btn-primary" onclick="FormRegistroAsistencia(' + this.CodPersonaProgramacion + ', ' + this.CodPersona + ');"><i class="fa fa-clock-o"></i> ' + BotonRegistrarAsistencia + '</a>' +
        '     </div>' +
        '   </div>' +
        ' </div>';
      });
  ...
  function FormRegistroAsistencia(CodPersonaProgramacion, CodPersona){
    var URL = URL_BASE + 'Registros/procGeneral';
    var Procedimiento = 'ProcPersonaProgramacion';
    var Parametros = CodPersonaProgramacion + '|' + CodPersona + '|' + COD_USUARIO;
    var Indice = 21;
    var Data = {
      parametros: Parametros,
      indice: Indice,
    ...
    $.post(URL, Data, function (Data) {
        console.log(Data);
        // var CodRespuesta = Data.split('|')[0];
        // var DesRespuesta = Data.split('|')[1];
        // var MensajeSunat = Data.split('|')[2];
        // var Tipo = '';
        // var Html = '';
        // if (CodRespuesta == "1") {
        //     ListarFacturacion();
        //     Tipo = 'success';
        //     Html = 'El comprobante: ' + ComprobanteSerie + ' ha sido anulado.\n Mensaje SUNAT: ' + MensajeSunat;
        // } else {
        //     Tipo = 'error'
        //     Html = DesRespuesta;
        // }
  ...
  function registrarAsistencia(){
    ...
    var Parametros =  $('#selectCodPersona').val() + '|' + 
                      ...
                      $('#txtComentario').val() ;
    //
    Parametros = '0|' + selectCodPersona.val() + '|' + COD_USUARIO;
    //
    var Indice = 21;
    var URL = URL_BASE + 'Registros/procGeneral';
    var Data = {
  ...
  function ListarPersona() {
    ...
    $.post(URL, Data, function (Data) {
      ...
      //
      $.each(Data, function(i) {
        OptionHTML += '<option value="' + this.CodPersona +'" >' + this.Alias + '</option>';
      });
