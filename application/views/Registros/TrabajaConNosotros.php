...
<div class="row">
   <div class="row wrapper border-bottom white-bg page-heading" style="padding-top: 20px;" >
        <div class="col-lg-12">
            <h2>Registros de trabaja con nosotros</h2>
        </div>
   </div>
   <br>
   <div class="row wrapper border-bottom white-bg page-heading" style="padding-top: 20px;" >
      <div class="col-lg-12">
        ...
        <div class="table-responsive">
          <table class="table table-bordered" id="tbregistrotrabajaconnosotros">
            <thead>
              <tr>
                <th>N</th>
                <th>NRO DOCUMENTO</th>
...
$(document).ready(function(){
    ...
    listarRegistrostrabajaconnosotros();


    $('#modalatencion').on('hidden.bs.modal', function () {
      $.each($('#tbregistrotrabajaconnosotros tbody > tr'),function(){
...
function listarRegistrostrabajaconnosotros(){
    
        $('#tbregistrotrabajaconnosotros tbody').empty();
        var parametros = '';
        var indice = 25;
        ...
        $.post(URL_BASE+'Registros/procGeneral', {
            ...
            strHTMLtabla += '<tr>'+
                                ...
                                '<td>'+ this.apellidos + ' ' + this.nombrescompletos +'</td>' +
                                '<td>'+ this.celular +'</td>' +
                                '<td>'+ this.correo +'</td>' +
                                '<td>'+ this.asunto +'</td>' +
                                '<td>'+ this.fechahora +'</td>' +
                                '<td>'+ '<button type="button" class="btn btn-primary btn-xs"><a style="color:white !important;" href=" '+ URL_BASE + '/assets/cvpostulantes/' + this.nombre_archivo + '" > <i class="fa fa-cloud-download"></i></a></button>' +'</td>' +
                                '<td>' + '<span class="label label-'+ ( Number(this.idestado) == 1 ? 'warning' : 'primary')+'">' + ( Number(this.idestado) == 1 ? 'ACTIVO' : 'ATENDIDO' ) + '</span>'  + '</td>' +
                                '<td>' + '<button data-comentarioatencion="'+ this.comentarioatencion +'" data-codestado="'+ this.idestado +'" data-codregistro="'+ this.idregtrabajaconnostros +'" onclick="abrirmodalatencion($(this).parent().parent(), $(this));" type="button" class="btn btn-success btn-xs" > <i class="fa fa-eye"></i></button>' + '</td>' +
                            '</tr>'; 
            });
            $('#tbregistrotrabajaconnosotros tbody').append(strHTMLtabla);
    },"JSON")
}
