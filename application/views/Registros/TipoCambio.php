...
<div class="row">
   <div class="row wrapper border-bottom white-bg page-heading" style="padding-top: 20px;" >
        ...
        <div class="col-lg-2">
            <label>&nbsp;&nbsp;</label>
            <input type="button" value="Registrar" onclick="abrirmodalregistro()" class="form-control primary" style="background-color: #1ab394; color: white;" />
   ...
   <div class="row wrapper border-bottom white-bg page-heading" style="padding-top: 20px;" >
      <div class="col-lg-12">
        ...
        <div class="table-responsive">
          <table class="table table-bordered" id="tbtipocambio">
            <thead>
              <tr>
                <th>N</th>
                <th>MONEDA</th>
...
<!-- Modal Slider imagen-->
<div class="modal fade" id="divmodaltipocambio" role="dialog">
        ...
        <div id="divCrearChip">
          <!-- <form id="FrmProducto" class="" enctype="multipart/form-data" method="post"> -->
            <div class="modal-body">
                <div class="form-group row"><label class="col-sm-4 col-form-label">Nombre</label>
                    <div class="col-sm-8">
                      <select id="selectnombremoneda" class="form-control" >
                        <option value="DOLAR" selected="selected" >DOLAR</option>
                      </select>
                    </div>
                </div>

                <div class="form-group row"><label class="col-sm-4 col-form-label">Precio compra</label>
                    <div class="col-sm-8"><input id="txtpreciocompra" onkeypress="return SoloDecimal(event, this);" type="text" class="form-control">
                    </div>
                </div>
                <br>
                <div class="form-group row"><label class="col-sm-4 col-form-label">Precio venta</label>
                    <div class="col-sm-8"><input id="txtprecioventa" onkeypress="return SoloDecimal(event, this);" type="text" class="form-control">
                    </div>
                </div>
            </div>

            <div class="modal-footer">
              <button type="submit" class="btn btn-w-m btn-primary" onclick="registranuevotipocambio();" >Guardar</button>
...
function abrirmodalregistro(){
  $('#selectnombremoneda').val('DOLAR');
  $('#txtpreciocompra').val('');
  $('#txtprecioventa').val('');
  $('#divmodaltipocambio').modal('show');

}

function listartipocambio(){
    
    $('#tbtipocambio tbody').empty();
    ...
    $.post(URL_BASE+'Registros/procGeneral', {
      ...
      $.each(respuesta,function(i){
        strHTMLtabla += '<tr>'+
                          '<td>'+ (i+1) +'</td>' +
                          '<td>'+ this.nombremoneda+'</td>' +
                          '<td>'+ this.cambiocompra +'</td>' +
                          '<td>'+ this.cambioventa +'</td>' +
                          '<td>'+ this.fechahoraregistro +'</td>' +
                          '<td>'+ (this.idestado == "1" ? '<span class="badge badge-primary">ACTIVO</span> ' : '<span class="badge badge-danger">INACTIVO</span>' ) +'</td>' +
                        '</tr>';
      });
      $('#tbtipocambio tbody').append(strHTMLtabla);

    },"JSON")
}

function registranuevotipocambio(){
    var selectnombre = $('#selectnombremoneda').val();
    var preciocompra = $('#txtpreciocompra').val();
    var precioventa = $('#txtprecioventa').val();

    var parametros = selectnombre + '|' + preciocompra + '|' +   precioventa;
